...
@include_once('../Core/baseModel.php');
    class TariandrModel extends BaseModel{
 ...
 */
        ...
        public  function show(){
            $sql = "BEGIN  pkgTariAndr.consultaTariAndr(:cod_hotel,:cod_alim,:res); END;";
            $conex = $this->db();
            ...
            while (($row = oci_fetch_array($cursor)) != false) {            
                ...
                $foo[] = new TariandrModel($row[0],$row[1],$row[2],$row[3],$row[4],$row[5],$row[6],$row[7],$row[8]);
            }
        ...
        public function insert(){
            ...
            $data=array();
            $sql = "BEGIN  pkgTariAndr.insertarTariAndr(:cod_hotel,:cod_tipAlim,:cod_acom,:fein_vig,:fefi_vig,:fein_exe,:fefi_exe,:val_tariandr,:user_tariandr); END;";
            $conex = $this->db();
            $stid = oci_parse($conex, $sql);
            oci_bind_by_name($stid, ':cod_hotel',$this->codHotel);
        ...
        function update(){
            $data=array();
            $sql = "BEGIN  pkgTariAndr.actualizarTariAndr(:cod_hotel,:cod_tipAlim,:cod_acom,:fein_vig,:fefi_vig,:fein_exe,:fefi_exe,:val_tariandr,:user_tariandr); END;";
            $conex = $this->db();
            $stid = oci_parse($conex, $sql);
            oci_bind_by_name($stid, ':cod_hotel',$this->codHotel);
            oci_bind_by_name($stid, ':cod_tipAlim',$this->codTipAlim);
