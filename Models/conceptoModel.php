 ...
 */
        ...
        public  function show(){
            $sql = "BEGIN  pkgConcepto.consultaConcepto(:cod_concepto,:nomb_concepto,:res); END;";
            $conex = $this->db();
            $stid = oci_parse($conex, $sql);
            $cursor= oci_new_cursor($conex);
            oci_bind_by_name($stid, ':cod_concepto',$this->code);
            oci_bind_by_name($stid, ':nomb_concepto',$this->name);
            ...
            while (($row = oci_fetch_array($cursor)) != false) {            
                ...
                $foo[] = new ConceptoModel($row[0],$row[1]);
            }
        ...
        public function insert(){
            ...
            $data=array();
            $sql = "BEGIN  pkgConcepto.insertarConcepto(:cod_concepto,:nomb_concepto); END;";
            $conex = $this->db();
            $stid = oci_parse($conex, $sql);
            oci_bind_by_name($stid, ':cod_concepto',$this->code);
            oci_bind_by_name($stid, ':nomb_concepto',$this->name);
        ...
        function update(){
            $data=array();
            $sql = "BEGIN  pkgConcepto.actualizarConcepto(:cod_concepto,:nomb_concepto); END;";
            $conex = $this->db();
            $stid = oci_parse($conex, $sql);
            oci_bind_by_name($stid, ':cod_concepto',$this->code);
            oci_bind_by_name($stid, ':nomb_concepto',$this->name);
