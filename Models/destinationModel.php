 ...
 */
        ...
        public  function show(){
            $sql = "BEGIN  pkgDestino.consultaDestino(:cod_destino,:nomb_destino,:res); END;";
            $conex = $this->db();
            $stid = oci_parse($conex, $sql);
            $cursor= oci_new_cursor($conex);
            oci_bind_by_name($stid, ':cod_destino',$this->code);
            oci_bind_by_name($stid, ':nomb_destino',$this->name);
            ...
            while (($row = oci_fetch_array($cursor)) != false) {            
                ...
                $foo[] = new DestinationModel($row[0],$row[1]);
            }
        ...
        public function insert(){
            ...
            $data=array();
            $sql = "BEGIN  pkgDestino.insertarDestino(:cod_destino,:nomb_destino); END;";
            $conex = $this->db();
            $stid = oci_parse($conex, $sql);
            oci_bind_by_name($stid, ':cod_destino',$this->code);
            oci_bind_by_name($stid, ':nomb_destino',$this->name);
        ...
        function update(){
            $data=array();
            $sql = "BEGIN  pkgDestino.actualizarDestino(:cod_destino,:nomb_destino); END;";
            $conex = $this->db();
            $stid = oci_parse($conex, $sql);
            oci_bind_by_name($stid, ':cod_destino',$this->code);
            oci_bind_by_name($stid, ':nomb_destino',$this->name);
