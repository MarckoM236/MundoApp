...
@include_once('../Core/baseModel.php');
    class TaricartModel extends BaseModel{
 ...
 */
        ...
        public  function show(){
            ...
            while (($row = oci_fetch_array($cursor)) != false) {            
                ...
                $foo[] = new TaricartModel($row[0],$row[1],$row[2],$row[3],$row[4],$row[5],$row[6],$row[7],$row[8]);
            }
        ...
        public function insert(){
            ...
            $data=array();
            $sql = "BEGIN  pkgTariCart.insertarTariCart(:cod_hotel,:cod_tipAlim,:cod_acom,:fein_vig,:fefi_vig,:fein_exe,:fefi_exe,:val_taricart,:user_taricart); END;";
            $conex = $this->db();
            $stid = oci_parse($conex, $sql);
            oci_bind_by_name($stid, ':cod_hotel',$this->codHotel);
        ...
        function update(){
            $data=array();
            $sql = "BEGIN  pkgTariCart.actualizarTariCart(:cod_hotel,:cod_tipAlim,:cod_acom,:fein_vig,:fefi_vig,:fein_exe,:fefi_exe,:val_taricart,:user_taricart); END;";
            $conex = $this->db();
            $stid = oci_parse($conex, $sql);
            oci_bind_by_name($stid, ':cod_hotel',$this->codHotel);
            oci_bind_by_name($stid, ':cod_tipAlim',$this->codTipAlim);
