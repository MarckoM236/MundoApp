...
@include_once('../Core/baseModel.php');
    class ReceandrModel extends BaseModel{
 ...
 */
        ...
        public function insert(){
            ...
            $data=array();
            $sql = "BEGIN  pkgReceAndr.insertarReceAndr(:cod_oper,:receptivo,:cod_hotel,:zona,:hoin_diurno,:hofi_diurno,:hoin_nocturno,:hofi_nocturno,:diurno,:nocturno,:val_receandr,:user_receandr); END;";
            $conex = $this->db();
            $stid = oci_parse($conex, $sql);
            oci_bind_by_name($stid, ':cod_oper',$this->codOperator);
            oci_bind_by_name($stid, ':receptivo',$this->receptivo);
        ...
        function update(){
            $data=array();
            $sql = "BEGIN  pkgReceAndr.actualizarReceAndr(:cod_oper,:receptivo,:cod_hotel,:zona,:hoin_diurno,:hofi_diurno,:hoin_nocturno,:hofi_nocturno,:diurno,:nocturno,:val_receandr,:user_receandr); END;";
            $conex = $this->db();
            $stid = oci_parse($conex, $sql);
            oci_bind_by_name($stid, ':cod_oper',$this->codOperator);
            oci_bind_by_name($stid, ':receptivo',$this->receptivo);
